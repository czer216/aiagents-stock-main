...
class ThemePeerSelector:
    ...
    def _build_mainboard_universe(self, limit: Optional[int] = None) -> List[str]:
        ...
        for symbol, name in stock_map.items():
            ...
            if "ST" in name_text:
                continue
            if not self._is_mainboard_or_chinext(symbol):
                continue
...
""".strip()
    ...
    def _is_mainboard_or_chinext(self, symbol: str) -> bool:
        if not symbol or len(symbol) < 3:
            return False
        return symbol.startswith(("000", "001", "002", "003", "600", "601", "603", "605", "300", "301"))
