...
from infrastructure.notification.notification_service import notification_service  # 复用主程序的通知服务
from services.config_manager import config_manager  # 复用主程序的配置管理器
...
class SmartMonitorEngine:
    ...
    def _normalize_schedule_times(self, schedule_times: Optional[List[str]]) -> List[str]:
        out: List[str] = []
        seen = set()
        for item in schedule_times or []:
            s = str(item or '').strip()
    ...
    def _build_decision_from_db_row(self, row: Dict) -> Dict:
        levels = row.get('key_price_levels')
        ...
        return {
            ...
            'reasoning': str(row.get('reasoning') or '复用当日最近AI决策'),
            'position_size_pct': row.get('position_size_pct') if row.get('position_size_pct') is not None else 20,
            'stop_loss_pct': row.get('stop_loss_pct') if row.get('stop_loss_pct') is not None else 5.0,
            'take_profit_pct': row.get('take_profit_pct') if row.get('take_profit_pct') is not None else 10.0,
    ...
    def _extract_key_level(self, levels: Dict, *keys: str) -> Optional[float]:
        for k in keys:
            if k in levels:
                num = self._to_float(levels.get(k))
                if num is not None:
                    return num
        return None

    def _apply_price_trigger(self, decision: Dict, market_data: Dict, has_position: bool) -> Dict:
        out = dict(decision)
        raw_action = str(out.get('action') or 'HOLD').upper()
        current_price = self._to_float((market_data or {}).get('current_price'))
        levels = out.get('key_price_levels') or {}
        if not isinstance(levels, dict):
            levels = {}

        support = self._extract_key_level(levels, 'support', 'buy_price', 'entry', 'entry_price')
        resistance = self._extract_key_level(levels, 'resistance', 'sell_price', 'exit', 'exit_price')

        triggered_action = 'HOLD'
        trigger_reason = '未命中关键价位'

        if current_price is None:
            trigger_reason = '当前价格不可用'
        elif raw_action == 'BUY':
            if not has_position and support is not None and current_price <= support:
                triggered_action = 'BUY'
                trigger_reason = f'命中买入位({support:.2f})'
            elif support is None:
                trigger_reason = '缺少买入关键价位'
        elif raw_action == 'SELL':
            if has_position and resistance is not None and current_price >= resistance:
                triggered_action = 'SELL'
                trigger_reason = f'命中卖出位({resistance:.2f})'
            elif resistance is None:
                trigger_reason = '缺少卖出关键价位'
        else:
            trigger_reason = 'AI建议持有'

        out['raw_action'] = raw_action
        out['action'] = triggered_action
        out['price_trigger_reason'] = trigger_reason
    ...
    def analyze_stock(self, stock_code: str, auto_trade: bool = False,
                     ...
                     daily_strong_themes: Optional[List[str]] = None,
                     ai_schedule_enabled: bool = False,
                     ai_schedule_times: Optional[List[str]] = None,
                     ai_daily_max_calls: int = 3) -> Dict:
        """
        分析单只股票并做出决策
        ...
        try:
            ...
            normalized_times = self._normalize_schedule_times(ai_schedule_times)
            daily_limit = max(1, int(ai_daily_max_calls or 3))
            ...
            if ai_schedule_enabled:
                ...
                if normalized_times and now_hhmm in normalized_times:
                    existed_at_slot = self.db.get_ai_decision_by_stock_date_and_time(stock_code, today, now_hhmm)
                    if existed_at_slot:
                        latest_for_today = self.db.get_latest_ai_decision_by_stock_and_date(stock_code, today)
                        if latest_for_today:
                            decision = self._build_decision_from_db_row(latest_for_today)
                            ai_reused = True
                        else:
                            ai_waiting = True
                            ai_wait_reason = 'waiting_for_schedule_time'
                    else:
                        today_count = sum(
                            1 for t in normalized_times
                            if self.db.get_ai_decision_by_stock_date_and_time(stock_code, today, t)
                        )
            ...
            if ai_waiting:
                self.logger.info(f"[{stock_code}] 未到AI计划时间且当日无可复用决策，跳过本轮")
                return {
                    'success': False,
                    'error': '未到AI计划时间且当日无可复用决策',
                    'session_info': session_info,
                    'skipped': True,
                    'skip_reason': ai_wait_reason,
                }
            ...
            if should_call_ai:
                ai_result = self.deepseek.analyze_stock_and_decide(
                    ...
                    douban_author_id=douban_author_id,
                    douban_author_name=douban_author_name,
                    douban_pattern_version=int(douban_pattern_version or 0),
                ...
                if not decision:
                    ...
                    if latest_for_today:
                        ...
                        return {
                            ...
                            'session_info': session_info,
                            'skipped': True,
                            'skip_reason': 'waiting_for_schedule_time',
                        }
            ...
            decision = self._apply_price_trigger(
                decision=decision,
                market_data=market_data,
                has_position=has_position,
            )
            self.logger.info(
                f"[{stock_code}] 实时触发判定: raw={decision.get('raw_action')} -> exec={decision.get('action')} | "
                f"{decision.get('price_trigger_reason')}"
            )
            ...
            return {
                ...
                'execution_result': execution_result,
                'ai_reused': ai_reused,
                'ai_new_request': bool(should_call_ai),
            }
    ...
    def _send_notification(self, stock_code: str, stock_name: str,
                          ...
                          position_cost: float, session_info: Dict):
        """
        发送通知（使用主程序的通知服务）
        ...
        try:
            ...
            raw_action = str(decision.get('raw_action') or action).upper()
            price_trigger_reason = str(decision.get('price_trigger_reason') or '')
...
• 原始建议: {raw_action}
• 执行动作: {action_text}
• 触发依据: {price_trigger_reason or '关键价位命中'}
...
"""
            ...
            # 确保读取用户最新通知配置（包括飞书Webhook）
            if hasattr(self.notification, '_load_config'):
                self.notification.config = self.notification._load_config()

            # 强制智能盯盘优先使用飞书Webhook（地址仍来自用户配置）
            webhook_url = str(self.notification.config.get('webhook_url', '')).strip()
            if webhook_url:
                self.notification.config['webhook_enabled'] = True
                self.notification.config['webhook_type'] = 'feishu'
    ...
    def start_monitor(self, stock_code: str, check_interval: int = 300,
        ...
        thread = threading.Thread(
            ...
            args=(stock_code, check_interval, auto_trade, notify, stop_flag,
                 ...
                 douban_author_id, douban_author_name, int(douban_pattern_version or 0),
                 stock_concepts or [], daily_strong_themes or [],
                 bool(ai_schedule_enabled), ai_schedule_times or [], int(ai_daily_max_calls or 3)),
            daemon=True
        )
        ...
        while not stop_flag.is_set():
            try:
                ...
                result = self.analyze_stock(
                    ...
                    douban_author_id=douban_author_id,
                    douban_author_name=douban_author_name,
                    douban_pattern_version=douban_pattern_version,
                    stock_concepts=stock_concepts or [],
                    daily_strong_themes=daily_strong_themes or [],
                    ai_schedule_enabled=ai_schedule_enabled,
                    ai_schedule_times=ai_schedule_times or [],
                    ai_daily_max_calls=ai_daily_max_calls,
                )
