...
class SmartMonitorDB:
    ...
    def _init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS monitor_tasks (
                ...
                position_date TEXT,
                douban_author_id TEXT,
                douban_author_name TEXT,
                douban_pattern_version INTEGER,
                stock_concepts TEXT,
        ...
        try:
            cursor.execute("ALTER TABLE monitor_tasks ADD COLUMN douban_author_id TEXT")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE monitor_tasks ADD COLUMN douban_author_name TEXT")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE monitor_tasks ADD COLUMN douban_pattern_version INTEGER")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE monitor_tasks ADD COLUMN stock_concepts TEXT")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE monitor_tasks ADD COLUMN ai_schedule_enabled INTEGER DEFAULT 0")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE monitor_tasks ADD COLUMN ai_schedule_times TEXT")
        except sqlite3.OperationalError:
            pass

        try:
            cursor.execute("ALTER TABLE monitor_tasks ADD COLUMN ai_daily_max_calls INTEGER DEFAULT 3")
        except sqlite3.OperationalError:
            pass
    ...
    def add_monitor_task(self, task_data: Dict) -> int:
        ...
        cursor.execute('''
            ...
            (task_name, stock_code, stock_name, enabled, check_interval,
             ...
             douban_author_id, douban_author_name, douban_pattern_version, stock_concepts,
             ai_schedule_enabled, ai_schedule_times, ai_daily_max_calls)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            ...
            task_data.get('douban_author_id'),
            task_data.get('douban_author_name'),
            task_data.get('douban_pattern_version'),
            task_data.get('stock_concepts'),
            task_data.get('ai_schedule_enabled', 0),
            task_data.get('ai_schedule_times'),
            task_data.get('ai_daily_max_calls', 3)
        ))
    ...
    def update_monitor_task(self, stock_code: str, task_data: Dict):
        ...
        if 'douban_author_id' in task_data:
            update_fields.append('douban_author_id = ?')
            values.append(task_data['douban_author_id'])

        if 'douban_author_name' in task_data:
            update_fields.append('douban_author_name = ?')
            values.append(task_data['douban_author_name'])

        if 'douban_pattern_version' in task_data:
            update_fields.append('douban_pattern_version = ?')
            values.append(task_data['douban_pattern_version'])

        if 'stock_concepts' in task_data:
            update_fields.append('stock_concepts = ?')
            values.append(task_data['stock_concepts'])

        if 'ai_schedule_enabled' in task_data:
            update_fields.append('ai_schedule_enabled = ?')
            values.append(task_data['ai_schedule_enabled'])

        if 'ai_schedule_times' in task_data:
            update_fields.append('ai_schedule_times = ?')
            values.append(task_data['ai_schedule_times'])

        if 'ai_daily_max_calls' in task_data:
            update_fields.append('ai_daily_max_calls = ?')
            values.append(task_data['ai_daily_max_calls'])
    ...
    def get_latest_ai_decision_by_stock_and_date(self, stock_code: str, date_str: str) -> Optional[Dict]:
        """获取某股票某日最近一条AI决策"""
        conn = sqlite3.connect(self.db_file)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT * FROM ai_decisions
            WHERE stock_code = ? AND substr(decision_time, 1, 10) = ?
            ORDER BY decision_time DESC
            LIMIT 1
        ''', (stock_code, date_str))
        row = cursor.fetchone()
        ...
        if not row:
            return None
        d = dict(row)
        d['key_price_levels'] = json.loads(d['key_price_levels']) if d.get('key_price_levels') else {}
        d['market_data'] = json.loads(d['market_data']) if d.get('market_data') else {}
        d['account_info'] = json.loads(d['account_info']) if d.get('account_info') else {}
        return d

    def get_ai_decision_by_stock_date_and_time(self, stock_code: str, date_str: str, hhmm: str) -> Optional[Dict]:
        """获取某股票某日在指定HH:MM的一条AI决策（同分钟）"""
        conn = sqlite3.connect(self.db_file)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        cursor.execute('''
            ...
            WHERE stock_code = ?
              ...
              AND substr(decision_time, 12, 5) = ?
            ORDER BY decision_time DESC
            LIMIT 1
        ''', (stock_code, date_str, hhmm))
        row = cursor.fetchone()
        conn.close()
