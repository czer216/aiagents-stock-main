...
from infrastructure.external.tushare_client import create_tushare_pro
...
class SmartMonitorDataFetcher:
    ...
    def __init__(self, use_tdx: bool = None, tdx_base_url: str = None):
        ...
        if tushare_token:
            try:
                self.ts_pro = create_tushare_pro(token=tushare_token)
                if self.ts_pro is not None:
                    self.logger.info("Tushare备用数据源初始化成功")
                else:
                    self.logger.warning("Tushare Token无效，初始化失败")
            except Exception as e:
    ...
    def _get_realtime_quote_from_tushare(self, stock_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            # 方法2: 降级使用更基础的stock_basic+daily
            try:
                # 获取股票名称
                stock_basic = self.ts_pro.stock_basic(ts_code=ts_code, fields='name')
                stock_name = stock_basic.iloc[0]['name'] if not stock_basic.empty else 'N/A'
                
                # 使用daily获取最近日线（避免额外地址配置不一致）
                df = self.ts_pro.daily(
                    ts_code=ts_code,
                    end_date=datetime.now().strftime('%Y%m%d'),
                ...
                if df is not None and not df.empty:
                    ...
                    self.logger.info(f"✅ Tushare降级成功（daily接口），获取到 {stock_code} 数据")
                ...
                self.logger.warning(f"Tushare daily接口失败: {str(e)[:100]}")
