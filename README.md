...
系统已增加登录鉴权，分为两类角色：
- 管理员（admin）：可使用全部功能，可编辑并保存环境配置（.env）。
- 普通用户（user）：可使用分析与监控功能，但环境配置仅可查看，不能保存或重置。

首次启动：
1. 若系统中不存在管理员账号，会自动进入“初始化管理员”页面。
2. 设置管理员用户名和密码（密码至少6位）后，使用该账号登录。

权限规则：
- 所有页面都需要登录后访问。
- “环境配置”页面保存/重置操作仅管理员可执行（后端已做强校验，不仅是按钮隐藏）。

忘记密码处理（本地部署）：
- 当前账号存储在 `auth.db`。
- 可通过 sqlite 手工重置指定用户密码哈希，或删除 `auth.db` 后重建管理员。
