...
class StockAnalysisAgents:
    ...
    def _build_ths_context_text(self, stock_info: Dict) -> str:
        """构建概念板块与同花顺历史异动上下文"""
        ...
        if isinstance(abnormal_data, dict) and abnormal_data.get("has_data"):
            ...
            lines.append(
                f"- 同花顺最近7天历史异动：{summary.get('total_records', 0)}条，"
...
"""
        ths_context_text = self._build_ths_context_text(stock_info)
...
{self._build_ths_context_text(stock_info)}
{sentiment_data_text}
...
请恢复并使用会议记录模板输出，格式如下（保留标题与字段）：
# {stock_info.get('name', 'N/A')}（{stock_info.get('symbol', 'N/A')}）投资决策团队会议记录
会议时间：{current_time}
主持人：{meeting_chair}
参会人员：{', '.join(participants)}

然后再给出详细讨论正文（对话形式），体现专业团队的思辨过程。
注意：只讨论参与分析的分析师观点；会议时间必须使用当前系统时间，不要写历史年份。
"""
        
        messages = [
            {"role": "system", "content": "你需要模拟一场专业的投资团队讨论会议，体现不同角色的观点碰撞和最终共识形成。必须以用户给定的当前系统时间为准，不得编造旧日期。"},
