...
from infrastructure.external.miniqmt_interface import miniqmt, get_miniqmt_status, QuantStrategyConfig
...
def display_monitor_status():
    ...
    with st.expander("⚙️ 快拉参数配置", expanded=False):
        ...
        with c1:
            ...
            mainboard_only = st.checkbox("仅主板", value=bool(rr_cfg.get('mainboard_only', True)))
        with c2:
            ...
            exclude_st = st.checkbox("排除ST（预留）", value=bool(rr_cfg.get('exclude_st', True)))
        with c3:
        ...
        st.caption(
            f"生效配置: interval={rr_cfg['interval_sec']}s, batch={rr_cfg['batch_size']}, 主板={rr_cfg.get('mainboard_only', True)}, "
