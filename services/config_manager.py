...
class ConfigManager:
    ...
    def __init__(self, env_file: str = ".env"):
        ...
        self.default_config = {
            ...
            "DOUBAN_COOKIE": {
                "value": "",
                "description": "豆瓣Cookie（用于抓取指定作者帖子）",
                "required": False,
                "type": "password"
            },
        }
    ...
    def write_env(self, config: Dict[str, str]) -> bool:
        ...
        try:
            ...
            # MySQL配置
            lines.append("# ========== MySQL数据库配置 ==========")
            lines.append(f'MYSQL_HOST="{final_config.get("MYSQL_HOST", "127.0.0.1")}"')
            lines.append(f'MYSQL_PORT="{final_config.get("MYSQL_PORT", "3306")}"')
            lines.append(f'MYSQL_USER="{final_config.get("MYSQL_USER", "root")}"')
            lines.append(f'MYSQL_PASSWORD="{final_config.get("MYSQL_PASSWORD", "")}"')
            lines.append(f'MYSQL_DATABASE="{final_config.get("MYSQL_DATABASE", "aiagents_stock")}"')
            lines.append(f'MYSQL_CHARSET="{final_config.get("MYSQL_CHARSET", "utf8mb4")}"')
            lines.append(f'MYSQL_AUTO_CREATE_DATABASE="{final_config.get("MYSQL_AUTO_CREATE_DATABASE", "true")}"')
            lines.append("")
            ...
            # 豆瓣抓取配置
            lines.append("# ========== 豆瓣抓取配置（可选）==========")
            lines.append(f'DOUBAN_COOKIE="{final_config.get("DOUBAN_COOKIE", "")}"')
            ...
            known_keys = {
                ...
                "MYSQL_DATABASE", "MYSQL_CHARSET", "MYSQL_AUTO_CREATE_DATABASE",
                ...
                "WEBHOOK_URL", "WEBHOOK_KEYWORD", "DOUBAN_COOKIE"
            }
