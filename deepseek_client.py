...
硬性约束（必须遵守）：
1) `entry_range` 必须是纯数字区间格式：`xx.xx-yy.yy`（不得带“约/元/<br>”）。
2) `entry_range` 的中位数必须围绕“当前价格锚点”，偏离不得超过 10%。
3) 若有7日高低点锚点：`entry_range` 必须落在 `[近7日最低价*0.97, 近7日最高价*1.03]` 范围内。
4) `take_profit`、`stop_loss` 必须是可解析数字（可带单位），且满足：
   `stop_loss < entry_range下沿 < entry_range上沿 < take_profit`。
5) 若锚点缺失或冲突，先使用当前价格与MA20做兜底，不得凭空给出明显偏离的价位。
