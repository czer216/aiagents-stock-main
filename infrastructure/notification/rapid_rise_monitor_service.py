...
class RapidRiseMonitorService:
    ...
    def __init__(self):
        ...
        self.mainboard_only = os.getenv('RAPID_RISE_MAINBOARD_ONLY', 'true').lower() == 'true'
        self.exclude_st = os.getenv('RAPID_RISE_EXCLUDE_ST', 'true').lower() == 'true'
    ...
    def get_runtime_config(self) -> Dict:
        return {
            ...
            'mainboard_only': self.mainboard_only,
    ...
    def update_runtime_config(
        ...
        self.mainboard_only = bool(mainboard_only)
    ...
    def _is_mainboard_code(self, code: str) -> bool:
        ...
        if len(c) != 6 or not c.isdigit():
            return False
        return not (c.startswith('300') or c.startswith('301') or c.startswith('688') or c.startswith('4') or c.startswith('8'))

    def _filter_symbols_for_scope(self, codes: List[str]) -> List[str]:
        out: List[str] = []
        for code in codes:
            c = str(code or '').strip()
            if len(c) != 6 or not c.isdigit():
                continue
            if self.mainboard_only and (not self._is_mainboard_code(c)):
                continue
            out.append(c)
        return out
    ...
    def _load_symbols(self) -> List[str]:
        try:
            ...
            normalized = [str(c).strip() for c in codes if str(c).strip().isdigit() and len(str(c).strip()) == 6]
            return self._filter_symbols_for_scope(normalized)
        except Exception:
            return []
    ...
    def _process_quotes(self, quotes: List[Dict]) -> int:
        ...
        for q in quotes:
            ...
            if not code:
                continue
            if self.mainboard_only and (not self._is_mainboard_code(code)):
                continue
    ...
    def _maybe_push_summary(self):
        ...
        scope_name = '主板' if self.mainboard_only else '全市场'
        title = f'{scope_name}快速拉升摘要'
        message = ' | '.join(lines)
        payload = {
            'symbol': 'MARKET',
            'name': scope_name,
    ...
    def _push_event(self, event: Dict, event_id: int):
        scope_name = '主板' if self.mainboard_only else '全市场'
        title = f"{scope_name}快速拉升 {event['symbol']} {event['trigger_level']}"
