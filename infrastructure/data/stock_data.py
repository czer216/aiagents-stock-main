...
class StockDataFetcher:
    ...
    def get_latest_indicators(self, df):
        ...
        try:
            ...
            latest_trade_date = ""
            try:
                latest_trade_date = pd.to_datetime(df.index[-1]).strftime("%Y-%m-%d")
            except Exception:
                latest_trade_date = ""
            recent_7 = df.tail(7) if len(df) >= 1 else df
            high_7d = None
            low_7d = None
            try:
                if 'High' in recent_7.columns and not recent_7['High'].dropna().empty:
                    high_7d = float(recent_7['High'].max())
                if 'Low' in recent_7.columns and not recent_7['Low'].dropna().empty:
                    low_7d = float(recent_7['Low'].min())
            except Exception:
                high_7d = None
                low_7d = None
