...
class StockMonitorDatabase:
    ...
    def init_database(self):
        ...
        # 创建索引（忽略错误，因为可能已存在或MySQL不支持某些语法）
        try:
        ...
        try:
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_rapid_rise_symbol_time ON rapid_rise_events(symbol, event_time)')
        except Exception:
            pass

        try:
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_rapid_rise_trade_time ON rapid_rise_events(trade_date, event_time)')
        except Exception:
            pass
        ...
        try:
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_rapid_push_status_time ON rapid_rise_push_records(status, created_at)')
        except Exception:
            pass
    ...
    def get_monitored_stock_count(self) -> int:
        """获取监测股票数量（轻量统计）"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def get_pending_notification_count(self) -> int:
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT COUNT(*) FROM notifications WHERE sent = FALSE')
        count = int(cursor.fetchone()[0] or 0)
        conn.close()
        return count
    ...
    def get_recent_rapid_rise_events(self, limit: int = 50) -> List[Dict]:
        ...
        cursor.execute('''
            SELECT id, trade_date, event_time, symbol, name, direction, last_price,
                   ...
                   trigger_level, theme_tokens, theme_hist_score, peer_linkage_score, payload_json, created_at
